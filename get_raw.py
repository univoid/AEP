...
def get_raw():
    # Edge input
    with open("csv/rawEdge.csv", "r") as inFile:
        line = inFile.readline()
        l = line.replace('|', '').replace('\"', '\'').split('}, {')
    with open("json/rawEdge.json", "w") as outFile:
        for st in l:
            stt = '{' + st + '}'
    ...
    # Vertex input
    # position
    text_file = open("csv/rawPos.csv", "r")
    ...
    for st in listPos:
        ...
        ansPos.append(temp)
    # other data
    with open("csv/rawVertex.csv", "r") as inFile:
        line = inFile.readline()
        l = line.replace('|', '').replace('\"', '\'').split('}, {')
    with open("json/rawVertex.json", "w") as outFile:
        i = 0
        for st in l:
            stt = '{' + st + '}'
            stt = stt.replace('{', '{\'').replace(':', '\':').replace(', ', ', \'')
